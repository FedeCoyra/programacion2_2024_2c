...
bool l_agregar(Lista lista, TipoElemento elemento) { // O(n)
    if (l_es_llena(lista)) {
        return false;
    }
    // Ahora lo agrego
...
bool l_borrar(Lista lista, int clave) { // O(n)
    if (l_es_vacia(lista)) {
        return false;
    }
...
bool l_eliminar(Lista lista, int pos) { // O(n)
    ...
    if (1 <= pos && pos <= l_longitud(lista)) {
        if (pos == 1) {
            ...
            // actual apunta al nodo en posici�n (pos - 1)
...
TipoElemento l_recuperar(Lista lista, int pos) { // O(n)
    if (pos > l_longitud(lista)) {
        return NULL;
    }
