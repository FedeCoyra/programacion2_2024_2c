...
int main() {
    ...
    Conjunto conjuntoResultante = cto_crear();
    // a. Union entre los conjuntos
    printf("a. Union entre los conjuntos\n");
    conjuntoResultante = cto_union(conjuntoA, conjuntoB);
    cto_mostrar(conjuntoResultante);
    // b. Interseccion entre los conjuntos
    printf("b. Interseccion entre los conjuntos\n");
    conjuntoResultante = cto_interseccion(conjuntoA, conjuntoB);
    cto_mostrar(conjuntoResultante);
    // c. Diferencia entre los conjuntos
    printf("c. Diferencia entre los conjuntos (A - B / B - A)\n");
    conjuntoResultante = cto_diferencia(conjuntoB, conjuntoA);
    cto_mostrar(conjuntoResultante);
    conjuntoResultante = cto_diferencia(conjuntoA, conjuntoB);
    cto_mostrar(conjuntoResultante);
    // d. Pertenencia en el conjunto
    printf("d. Pertenencia en el conjunto\n");
    char numeroEntrada[TAMANO_NUM];
    int numero = 0;
    bool valido;
    do {
        printf("Ingrese el numero que quiere buscar: ");
        fgets(numeroEntrada, TAMANO_NUM, stdin);
        valido = esEntero(numeroEntrada) && esEnter(numeroEntrada);
    } while (!valido);
    numero = convertirChar(numeroEntrada);

    bool perteneceA, perteneceB;
    perteneceA = cto_pertenece(conjuntoA, numero);
    perteneceB = cto_pertenece(conjuntoB, numero);

    if (perteneceA && perteneceB) {
        printf("La clave se encuentra en ambos conjuntos.\n");
    } else if (perteneceA) {
        printf("La clave solo se encuentra en el conjunto A.\n");
    } else if (perteneceB) {
        printf("La clave solo se encuentra en el conjunto B.\n");
    } else {
        printf("La clave no se encuentra en ningun conjunto.\n");
    }
