...
// funcion que verifica si un conjunto A es subconjunto de un conjunto B
bool esSubconjunto(Conjunto A, Conjunto B) {
    Conjunto elementosEnComun = cto_interseccion(A, B);
    if (cto_cantidad_elementos(elementosEnComun) == cto_cantidad_elementos(A)) {
        return true;
    }
    return false;
}

// funcion que verifica que los conjuntos sean diferentes
bool sonDiferentes(Conjunto A, Conjunto B) {
    Conjunto diferencia1 = cto_diferencia(A, B);
    Conjunto diferencia2 = cto_diferencia(B, A);
    if (!cto_es_vacio(diferencia1) || !cto_es_vacio(diferencia2)) {
        return true;
    }
    return false;
}

// funcion que verifica si un conjunto A es subconjunto propio de un conjunto B
bool esSubconjuntoPropio(Conjunto A, Conjunto B) {
    if (esSubconjunto(A, B) && sonDiferentes(A, B)) {
        return true;
    }
    return false;
}
...
int main() {
    ...
    if (esSubconjunto(conjuntoA, conjuntoB)) {
        if (sonDiferentes(conjuntoA, conjuntoB)) {
            printf("El conjunto A es subconjunto propio de B.\n");
        } else {
            printf("El conjunto A es igual a B (no es subconjunto propio).\n");
        }
    } else {
        printf("El conjunto A NO es subconjunto de B.\n");
    }

    // Verificamos si B es subconjunto de A
    if (esSubconjunto(conjuntoB, conjuntoA)) {
        if (sonDiferentes(conjuntoB, conjuntoA)) {
            printf("El conjunto B es subconjunto propio de A.\n");
        } else {
            printf("El conjunto B es igual a A (no es subconjunto propio).\n");
        }
    } else {
        printf("El conjunto B NO es subconjunto de A.\n");
    }
