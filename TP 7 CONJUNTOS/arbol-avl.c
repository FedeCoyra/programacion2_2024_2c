...
NodoArbol avl_insertar_recursivo(ArbolAVL a, TipoElemento te, NodoArbol pa, bool *inserto) {
    ...
    if (te->clave < pa->datos->clave) {
        ...
        // Es una inserci�n de un elemento duplicado
    ...
    if (balanceState == DESBALANCEADO_IZQUIERDA) {
        if (te->clave < pa->hi->datos->clave) {
            // Caso rotaci�n derecha
            pa = avl_rotar_derecha(pa);
        } else {
            // Caso rotaci�n izquierda-derecha
    ...
    if (balanceState == DESBALANCEADO_DERECHA) {
        if (te->clave > pa->hd->datos->clave) {
            // Caso rotaci�n izquierda
            pa = avl_rotar_izquierda(pa);
        } else {
            // Caso rotaci�n derecha-izquierda
...
//            return temp;
            ...
            // El nodo tiene 2 hijos, buscamos el sucesor en in-orden (o sea el menor del sub�rbol de la derecha)
    ...
    // Actualizar altura y re-balancear el �rbol de ser necesario
