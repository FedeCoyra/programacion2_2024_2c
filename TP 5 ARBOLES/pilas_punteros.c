...
bool p_apilar(Pila pila, TipoElemento elemento) { // O(n)
    if (p_es_llena(pila)) {
        return false;
    }
...
TipoElemento p_desapilar(Pila pila) { // O(1)
    if (p_es_vacia(pila)) {
        return NULL;
    }
...
TipoElemento p_tope(Pila pila) { // O(1)
    if (p_es_vacia(pila)) {
        return NULL;
    }
    struct Nodo *tope_actual = pila->tope;
...
bool p_es_llena(Pila pila) { // O(n)
    return (longitud(pila) == TAMANIO_MAXIMO_PILAS);
}


void p_mostrar(Pila pila) { // O(n)
    if (p_es_vacia(pila)) {
        printf("PILA VACIA !!! \n");
        return;
    }
